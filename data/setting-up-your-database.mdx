...
![](/imgs/chapter6-import-git-repo.avif)

给你的项目取一个名字，然后点击 Deploy（部署）。

![](/imgs/chapter6-configure-project.avif)

太棒了！🎉 你的项目现在已经部署完成。

![](/imgs/chapter6-deployed-project.avif)
...
![Connect Store 屏幕显示了 Postgres 选项以及 KV、Blob 和 Edge Config](/imgs/chapter6-create-database.avif)
...
![数据库创建模态框显示了数据库名称和区域](/imgs/chapter6-database-region.avif)
...
![.env.local 选项卡显示了隐藏的数据库秘密](/imgs/chapter6-database-dashboard.avif)
...
![数据库屏幕显示了下拉列表，其中有四个表：users、customers、invoices 和 revenue](/imgs/chapter6-database-tables.avif)
...
在本地开发时你可能想使用本地搭建的 Postgres 数据库，但 `Vercel Postgres` 目前支持的并不是特别好，详情请参见 [扩展篇 1：Vercel Postgres 搭配本地数据库](/imgs/chapter17)
