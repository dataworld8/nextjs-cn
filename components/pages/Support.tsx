  ...
  return (
    <div className="mx-auto w-full max-w-screen-xl px-4 pt-16 md:px-8 md:pt-24">
      <div className="flex flex-col justify-center space-y-8 md:flex-row md:space-x-16 md:space-y-0">
        <div>
          <h2 className="mb-2 mt-0 text-sm font-normal text-slate-500 md:text-center">
            {content.contentSources.label}
          </h2>
          <div className="-mx-3 flex flex-wrap items-center">
            {content.contentSources.items.map(({ logo, label, supported, url }, index) => (
              <Link key={index} href={url} className="flex h-16 items-center p-3 dark:brightness-200">
        ...
        <div>
          <h2 className="mb-2 mt-0 text-sm font-normal text-slate-500 md:text-center">{content.frameworks.label}</h2>
          <div className="-mx-3 flex flex-wrap items-center">
            {content.frameworks.items.map(({ logo, label, url, supported }, index) => (
              <Link key={index} href={url} className="flex h-16 items-center p-3 dark:brightness-200">
                  ...
                  {/* <Tooltip>
                    <TooltipTrigger asChild className="flex h-full items-center">
                      <Image
                          src={content.logosPath + logo.file}
                          alt={label}
                          width={logo.width}
